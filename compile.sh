...
# compile TCP server
javac -target 1.8 -source 1.8 -d ../bin servers/*.java # compile tcp server, satisfy server version
if [ $? -ne 0 ]
then
...
# compile UDP server
javac -target 1.8 -source 1.8 -d ../bin com/udpmeasurement/*.java #satisfy server version
if [ $? -ne 0 ]
then
